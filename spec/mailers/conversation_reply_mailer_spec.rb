...
RSpec.describe ConversationReplyMailer do
  describe 'reply' do
    ...
    context 'with summary' do
      ...
      let(:private_message) { create(:message, account: account, content: 'Essa é uma mensagem privada', conversation: conversation) }
      ...
      it 'renders the default subject' do
        expect(mail.subject).to eq("[##{message.conversation.display_id}] Novas mensagens nessa conversa")
      end
    ...
    context 'without summary' do
      ...
      let(:private_message) do
        create(:message,
               content: 'Essa é uma mensagem privada',
               conversation: conversation,
               account: account,
      ...
      it 'renders the default subject' do
        expect(mail.subject).to eq("[##{message_2.conversation.display_id}] Novas mensagens nessa conversa")
      end
    ...
    context 'when inbox email address is available' do
      let(:inbox) { create(:inbox, account: account, email_address: 'noreply@virtuacrm.com') }
