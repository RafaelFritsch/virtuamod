...
RSpec.describe 'Confirmation Instructions' do
  describe :notify do
    ...
    it 'does not refer to the inviter and their account' do
      expect(mail.body).to_not match('te convidou para expirimentar o VirtuaCRM')
      expect(mail.body).to match('Temos um conjunto de poderorar ferramentas para você usar.')
    end
    ...
    context 'when there is an inviter' do
      ...
      it 'refers to the inviter and their account' do
        expect(mail.body).to match(
          "#{CGI.escapeHTML(inviter_val.name)}, with #{CGI.escapeHTML(account.name)}, te convidou para expirimentar o VirtuaCRM."
        )
        expect(mail.body).not_to match('Temos um conjunto de poderorar ferramentas para você usar..')
      end
